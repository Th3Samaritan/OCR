...
> Original Colab/Kaggle notebook demo (`unlimited_ocr_demo.ipynb`). Preserved from the
> initial project README. **Note:** this notebook used Gemini for the structured-extraction
> step; the backend platform (see the top-level `README.md`) uses Claude. Reconcile the
> extraction provider before treating either as canonical.
